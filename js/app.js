...
function renderVersiculos(){
  ...
  orden.forEach(cat=>{
    ...
    c.className='mc';
    c.innerHTML='<div class="mh" onclick="togM(this)"><span class="tdot" style="background:'+color+'"></span><span class="mn">'+cat+'</span><span style="font-size:11px;color:var(--text3)">'+items.length+'</span><span class="mchev">▼</span></div><div class="mb"><div class="vg" style="margin-top:2px">'+cards+'</div></div>';
    g.appendChild(c);
  });
}
...
function renderOraciones(){const g=document.getElementById('og');ORACIONES.forEach(o=>{const c=document.createElement('div');c.className='oc';c.innerHTML='<div class="oh" onclick="togO(this)"><span style="color:var(--gold);font-size:14px">✝</span><span class="otit">'+o.t+'</span><span class="otype">'+o.tp+'</span><span class="ochev">▼</span></div><div class="ob"><div class="otxt">'+o.tx+'</div><div class="oorig">— '+o.o+'</div></div>';g.appendChild(c);});}
...
function renderSantosGaleria(){
  ...
  SANTOS_DESTACADOS.forEach(s=>{
    ...
    c.innerHTML='<div class="santo-flip-inner">'
      ...
      +'<div class="santo-cara santo-dorso"><div class="santo-dorso-nombre">'+s.nombre+'</div><div class="santo-dorso-bio">'+s.bio+'</div><div class="santo-dorso-oracion">'+s.oracion+'</div></div>'
      +'</div>';
...
function renderSantos(){const g=document.getElementById('sag');SANTOS.forEach(m=>{const c=document.createElement('div');c.className='mc';const sh=m.s.map(s=>'<div class="si"><span class="sd">'+s.d+'</span><div><div class="sn">'+s.n+'</div><div class="snota">'+s.nota+'</div></div></div>').join('');c.innerHTML='<div class="mh" onclick="togM(this)"><span style="color:var(--gold);font-size:12px">✦</span><span class="mn">'+m.m+'</span><span style="font-size:11px;color:var(--text3)">'+m.s.length+' santos</span><span class="mchev">▼</span></div><div class="mb">'+sh+'</div>';g.appendChild(c);});}
